(($) ->
  ...
  _utilVars =
    ...
    sampleBtRemove : '<a data-index="{index}" class="btn btn-danger" title="Remove this row"><i class="{icon-remove}"></i></a>'
    sampleBtAdd : '<buttton class="{bt-class}" type="button" title="Add new row"><i class="{icon-add}"></i> </button>'
  ...
  _generateNewRow = (tbWhole) ->
    ...
    for field in optColumns
      ...
      # type select
        ...
        _row = _genRowSelectNewMode(settings, attrs)
      ...
      for field in optColumns
        if mode == 'edit'
          if field.type in settings.supportedTypes
            ...
            _row = _genRowSelectNewMode(settings, field)
  ...
  _genRowSelectNewMode = (settings, attrs) ->
    ...
    if attrs.options.length > 0
      ...
      _row.append('<option value="">' + settings.textEmptyOptionCombo + '</option>')
    _row
